...
std::string engine_info(bool to_uci) {
    ...
    date >> month >> day >> year;
    ss << "Polyfish "
       << std::setw(2) << std::setfill('0') << year.substr(2)
       << std::setw(2) << std::setfill('0') << (1 + months.find(month) / 4)
       << std::setw(2) << std::setfill('0') << day
       << (to_uci ? "\nid author " : " by ") << "Khalid Omar";
#else
    std::stringstream ss;
...
#if defined(POLYFISH)
/*static*/ CommandLine* Utility::cli = nullptr;

/*static*/ void Utility::init(CommandLine* _cli)
{
    cli = _cli;
...
/*static*/ std::string Utility::unquote(const std::string& s)
{
    std::string s1 = s;
...
/*static*/ bool Utility::is_empty_filename(const std::string &fn)
{
    if (fn.empty())
        return true;
...
/*static*/ std::string Utility::fix_path(const std::string& p)
{
...
/*static*/ std::string Utility::combine_path(const std::string& p1, const std::string& p2)
{
...
/*static*/ std::string Utility::map_path(const std::string& p)
{
    if (is_empty_filename(p))
        return p;
...
/*static*/ size_t Utility::get_file_size(const std::string& f)
{
...
/*static*/ bool Utility::is_same_file(const std::string& f1, const std::string& f2)
{
    return map_path(f1) == map_path(f2);
}

/*static*/ std::string Utility::format_bytes(uint64_t bytes, int decimals)
{
...
/*static*/ std::string Utility::format_string(const char* const fmt, ...)
{
